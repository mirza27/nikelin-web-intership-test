  ...
  async function fetchAndGenerateExcel(year: number) {
    try {
      ...
      const formattedData = data.map((item: Booking) => ({
        "Tanggal Pemesanan": formatDate(new Date(item.createdAt).toISOString()),
        "Booking ID": item.id,
        ...
        "Tanggal Mulai": formatDate(new Date(item.startDate).toISOString()),
        "Tanggal Selesai": formatDate(new Date(item.endDate).toISOString()),
